...
// forward references
template <int ALGO> __global__ void nv_scrypt_core_kernelA(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, uint32_t *g_idata, int begin, int end);
template <int ALGO, int TEX_DIM> __global__ void nv_scrypt_core_kernelB(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, uint32_t *g_odata, int begin, int end);
template <int ALGO> __global__ void nv_scrypt_core_kernelA_LG(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, uint32_t *g_idata, int begin, int end, unsigned int LOOKUP_GAP);
template <int ALGO, int TEX_DIM> __global__ void nv_scrypt_core_kernelB_LG(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, uint32_t *g_odata, int begin, int end, unsigned int LOOKUP_GAP);
...
extern cudaTextureObject_t texRef1D_4_V;
extern cudaTextureObject_t texRef2D_4_V;
extern cudaArray_t cuArray;
...
bool NVKernel::bindtexture_1D(uint32_t *d_V, size_t size)
{
    ...
    struct cudaTextureDesc texRef1D_4_VDesc;
    memset(&texRef1D_4_VDesc, 0, sizeof(struct cudaTextureDesc));
    texRef1D_4_VDesc.normalizedCoords = 0;
    texRef1D_4_VDesc.filterMode = cudaFilterModePoint;
    texRef1D_4_VDesc.addressMode[0] = cudaAddressModeClamp;
    texRef1D_4_VDesc.readMode = cudaReadModeElementType;

    struct cudaResourceDesc res1Desc;
    memset(&res1Desc, 0, sizeof(res1Desc));
    res1Desc.resType = cudaResourceTypeLinear;
    res1Desc.res.linear.devPtr = d_V;
    res1Desc.res.linear.desc = channelDesc4;
    res1Desc.res.linear.sizeInBytes = size;

    checkCudaErrors(cudaCreateTextureObject(&texRef1D_4_V, &res1Desc, &texRef1D_4_VDesc, NULL));
    return true;
}
...
bool NVKernel::bindtexture_2D(uint32_t *d_V, int width, int height, size_t pitch)
{
    ...
    struct cudaTextureDesc texRef2D_4_VDesc;
    memset(&texRef2D_4_VDesc, 0, sizeof(struct cudaTextureDesc));
    texRef2D_4_VDesc.normalizedCoords = 0;
    texRef2D_4_VDesc.filterMode = cudaFilterModePoint;
    texRef2D_4_VDesc.addressMode[0] = cudaAddressModeClamp;
    texRef2D_4_VDesc.addressMode[1] = cudaAddressModeClamp;
    texRef2D_4_VDesc.readMode = cudaReadModeElementType;

    cudaMallocArray(&cuArray, &channelDesc4, width, height);
    cudaMemcpy2DToArray(cuArray, 0, 0, d_V, pitch, width * sizeof(uint32_t), height, cudaMemcpyDeviceToDevice);

    struct cudaResourceDesc res2Desc;
    memset(&res2Desc, 0, sizeof(res2Desc));
    res2Desc.resType = cudaResourceTypeArray;
    res2Desc.res.array.array = cuArray;

    checkCudaErrors(cudaCreateTextureObject(&texRef2D_4_V, &res2Desc, &texRef2D_4_VDesc, NULL));
    return true;
}

bool NVKernel::unbindtexture_1D()
{
    checkCudaErrors(cudaDestroyTextureObject(texRef1D_4_V));
    return true;
}

bool NVKernel::unbindtexture_2D()
{
    checkCudaErrors(cudaDestroyTextureObject(texRef2D_4_V));
...
bool NVKernel::run_kernel(dim3 grid, dim3 threads, int WARPS_PER_BLOCK, int thr_id, cudaStream_t stream, uint32_t* d_idata, uint32_t* d_odata, unsigned int N, unsigned int LOOKUP_GAP, bool interactive, bool benchmark, int texture_cache)
{
    ...
    do 
    {
        if (LOOKUP_GAP == 1)
            switch(opt_algo) {
                case ALGO_SCRYPT:      nv_scrypt_core_kernelA<ALGO_SCRYPT>     <<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_idata, pos, min(pos+batch, N)); break;
                case ALGO_SCRYPT_JANE: nv_scrypt_core_kernelA<ALGO_SCRYPT_JANE><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_idata, pos, min(pos+batch, N)); break;
            }
        else
            switch(opt_algo) {
                case ALGO_SCRYPT:      nv_scrypt_core_kernelA_LG<ALGO_SCRYPT>     <<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_idata, pos, min(pos+batch, N), LOOKUP_GAP); break;
                case ALGO_SCRYPT_JANE: nv_scrypt_core_kernelA_LG<ALGO_SCRYPT_JANE><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_idata, pos, min(pos+batch, N), LOOKUP_GAP); break;
            }
    ...
    do
    {
        if (LOOKUP_GAP == 1) {
            if (texture_cache == 0) switch(opt_algo) {
                case ALGO_SCRYPT:      nv_scrypt_core_kernelB<ALGO_SCRYPT     ,0><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N)); break;
                case ALGO_SCRYPT_JANE: nv_scrypt_core_kernelB<ALGO_SCRYPT_JANE,0><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N)); break;
            }
            else if (texture_cache == 1) switch(opt_algo) {
                case ALGO_SCRYPT:      nv_scrypt_core_kernelB<ALGO_SCRYPT     ,1><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N)); break;
                case ALGO_SCRYPT_JANE: nv_scrypt_core_kernelB<ALGO_SCRYPT_JANE,1><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N)); break;
            }
            else if (texture_cache == 2) switch(opt_algo) {
                case ALGO_SCRYPT:      nv_scrypt_core_kernelB<ALGO_SCRYPT     ,2><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N)); break;
                case ALGO_SCRYPT_JANE: nv_scrypt_core_kernelB<ALGO_SCRYPT_JANE,2><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N)); break;
            }
        } else {
            if (texture_cache == 0) switch(opt_algo) {
                case ALGO_SCRYPT:      nv_scrypt_core_kernelB_LG<ALGO_SCRYPT     ,0><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N), LOOKUP_GAP); break;
                case ALGO_SCRYPT_JANE: nv_scrypt_core_kernelB_LG<ALGO_SCRYPT_JANE,0><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N), LOOKUP_GAP); break;
            }
            else if (texture_cache == 1) switch(opt_algo) {
                case ALGO_SCRYPT:      nv_scrypt_core_kernelB_LG<ALGO_SCRYPT     ,1><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N), LOOKUP_GAP); break;
                case ALGO_SCRYPT_JANE: nv_scrypt_core_kernelB_LG<ALGO_SCRYPT_JANE,1><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N), LOOKUP_GAP); break;
            }
            else if (texture_cache == 2) switch(opt_algo) {
                case ALGO_SCRYPT:      nv_scrypt_core_kernelB_LG<ALGO_SCRYPT     ,2><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N), LOOKUP_GAP); break;
                case ALGO_SCRYPT_JANE: nv_scrypt_core_kernelB_LG<ALGO_SCRYPT_JANE,2><<< grid, threads, 0, stream >>>(texRef1D_4_V,texRef2D_4_V, d_odata, pos, min(pos+batch, N), LOOKUP_GAP); break;
            }
        }
...
template <int TEX_DIM> __device__ __forceinline__ void __transposed_read_BC(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, const uint4 *S, uint4 (&B)[4], uint4 (&C)[4], int spacing, int row)
{
    unsigned int laneId = __laneId();
    ...
    T1[7] = TEX_DIM==0 ? __ldg(loc) : TEX_DIM==1 ? tex1Dfetch<uint4>(texRef1D_4_V, loc-(uint4*)c_V[0]) : tex2D<uint4>(texRef2D_4_V, 0.5f + ((loc-(uint4*)c_V[0])%TEXWIDTH), 0.5f + ((loc-(uint4*)c_V[0])/TEXWIDTH));
    loc = &S[(spacing*2*(32*tile+4 ) + (lane8+7)%8 + 8*__shfl(row, 1, 8))];
    T1[6] = TEX_DIM==0 ? __ldg(loc) : TEX_DIM==1 ? tex1Dfetch<uint4>(texRef1D_4_V, loc-(uint4*)c_V[0]) : tex2D<uint4>(texRef2D_4_V, 0.5f + ((loc-(uint4*)c_V[0])%TEXWIDTH), 0.5f + ((loc-(uint4*)c_V[0])/TEXWIDTH));
    loc = &S[(spacing*2*(32*tile+8 ) + (lane8+6)%8 + 8*__shfl(row, 2, 8))];
    T1[5] = TEX_DIM==0 ? __ldg(loc) : TEX_DIM==1 ? tex1Dfetch<uint4>(texRef1D_4_V, loc-(uint4*)c_V[0]) : tex2D<uint4>(texRef2D_4_V, 0.5f + ((loc-(uint4*)c_V[0])%TEXWIDTH), 0.5f + ((loc-(uint4*)c_V[0])/TEXWIDTH));
    loc = &S[(spacing*2*(32*tile+12) + (lane8+5)%8 + 8*__shfl(row, 3, 8))];
    T1[4] = TEX_DIM==0 ? __ldg(loc) : TEX_DIM==1 ? tex1Dfetch<uint4>(texRef1D_4_V, loc-(uint4*)c_V[0]) : tex2D<uint4>(texRef2D_4_V, 0.5f + ((loc-(uint4*)c_V[0])%TEXWIDTH), 0.5f + ((loc-(uint4*)c_V[0])/TEXWIDTH));
    loc = &S[(spacing*2*(32*tile+16) + (lane8+4)%8 + 8*__shfl(row, 4, 8))];
    T1[3] = TEX_DIM==0 ? __ldg(loc) : TEX_DIM==1 ? tex1Dfetch<uint4>(texRef1D_4_V, loc-(uint4*)c_V[0]) : tex2D<uint4>(texRef2D_4_V, 0.5f + ((loc-(uint4*)c_V[0])%TEXWIDTH), 0.5f + ((loc-(uint4*)c_V[0])/TEXWIDTH));
    loc = &S[(spacing*2*(32*tile+20) + (lane8+3)%8 + 8*__shfl(row, 5, 8))];
    T1[2] = TEX_DIM==0 ? __ldg(loc) : TEX_DIM==1 ? tex1Dfetch<uint4>(texRef1D_4_V, loc-(uint4*)c_V[0]) : tex2D<uint4>(texRef2D_4_V, 0.5f + ((loc-(uint4*)c_V[0])%TEXWIDTH), 0.5f + ((loc-(uint4*)c_V[0])/TEXWIDTH));
    loc = &S[(spacing*2*(32*tile+24) + (lane8+2)%8 + 8*__shfl(row, 6, 8))];
    T1[1] = TEX_DIM==0 ? __ldg(loc) : TEX_DIM==1 ? tex1Dfetch<uint4>(texRef1D_4_V, loc-(uint4*)c_V[0]) : tex2D<uint4>(texRef2D_4_V, 0.5f + ((loc-(uint4*)c_V[0])%TEXWIDTH), 0.5f + ((loc-(uint4*)c_V[0])/TEXWIDTH));
    loc = &S[(spacing*2*(32*tile+28) + (lane8+1)%8 + 8*__shfl(row, 7, 8))];
    T1[0] = TEX_DIM==0 ? __ldg(loc) : TEX_DIM==1 ? tex1Dfetch<uint4>(texRef1D_4_V, loc-(uint4*)c_V[0]) : tex2D<uint4>(texRef2D_4_V, 0.5f + ((loc-(uint4*)c_V[0])%TEXWIDTH), 0.5f + ((loc-(uint4*)c_V[0])/TEXWIDTH));
...
template <int TEX_DIM> __device__ __forceinline__ void __transposed_xor_BC(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, const uint4 *S, uint4 (&B)[4], uint4 (&C)[4], int spacing, int row)
{
    uint4 BT[4], CT[4];
    __transposed_read_BC<TEX_DIM>(texRef1D_4_V, texRef2D_4_V, S, BT, CT, spacing, row);
...
template <int ALGO> __global__ void nv_scrypt_core_kernelA(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, uint32_t *g_idata, int begin, int end)
{
    ...
    if(i == 0) {
        __transposed_read_BC<0>(texRef1D_4_V, texRef2D_4_V,  (uint4*)g_idata, B, C, 1, 0);
        __transposed_write_BC(B, C, (uint4*)V, c_N); 
        ++i;
    } else
        __transposed_read_BC<0>(texRef1D_4_V, texRef2D_4_V,(uint4*)(V + (i-1)*32), B, C, c_N, 0);
...
template <int ALGO> __global__ void nv_scrypt_core_kernelA_LG(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, uint32_t *g_idata, int begin, int end, unsigned int LOOKUP_GAP)
{
    ...
    if(i == 0) {
        ...
        __transposed_read_BC<0>(texRef1D_4_V, texRef2D_4_V, (uint4*)(V + pos*32), B, C, c_spacing, 0);
...
template <int ALGO, int TEX_DIM> __global__ void nv_scrypt_core_kernelB(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, uint32_t *g_odata, int begin, int end)
{
    ...
    if(begin == 0) {
        __transposed_read_BC<TEX_DIM>(texRef1D_4_V, texRef2D_4_V, (uint4*)V, B, C, c_N, c_N_1);
        block_mixer<ALGO>(B, C); block_mixer<ALGO>(C, B);
    } else
        __transposed_read_BC<0>(texRef1D_4_V, texRef2D_4_V, (uint4*)g_odata, B, C, 1, 0);

    for (int i = begin; i < end; i++)  {
        int slot = C[0].x & c_N_1;
        __transposed_xor_BC<TEX_DIM>(texRef1D_4_V, texRef2D_4_V, (uint4*)(V), B, C, c_N, slot);
        block_mixer<ALGO>(B, C); block_mixer<ALGO>(C, B);
    }
...
template <int ALGO, int TEX_DIM> __global__ void nv_scrypt_core_kernelB_LG(cudaTextureObject_t texRef1D_4_V, cudaTextureObject_t texRef2D_4_V, uint32_t *g_odata, int begin, int end, unsigned int LOOKUP_GAP)
{
    ...
    if(begin == 0) {
      ...
      __transposed_read_BC<TEX_DIM>(texRef1D_4_V, texRef2D_4_V, (uint4*)V, B, C, c_spacing, pos);
      while(loop--) { block_mixer<ALGO>(B, C); block_mixer<ALGO>(C, B); }
    } else {
        __transposed_read_BC<TEX_DIM>(texRef1D_4_V, texRef2D_4_V, (uint4*)g_odata, B, C, 1, 0);
    }

    for (int i = begin; i < end; i++)  {
        int slot = C[0].x & c_N_1;
        int pos = slot/LOOKUP_GAP, loop = slot-pos*LOOKUP_GAP;
        uint4 b[4], c[4]; __transposed_read_BC<TEX_DIM>(texRef1D_4_V, texRef2D_4_V, (uint4*)(V), b, c, c_spacing, pos);
        while(loop--) { block_mixer<ALGO>(b, c); block_mixer<ALGO>(c, b); }
#pragma unroll 4
